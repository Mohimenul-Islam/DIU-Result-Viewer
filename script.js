async function fetchResult() {
  ...
  try {
    ...
    displayContactSection(resultContainer);
  }
}
...
function displayResult(data) {
  ...
  if (data.results.length > 0) {
    const { studentInfo, results } = data;

    let resultHtml = `
        <div class="card">
            <div class="text-center mb-8">
                <h2 class="text-2xl font-bold text-gray-800 mb-2">Result for Student ID: ${studentInfo.studentId}</h2>
                <p class="text-gray-600">Name: ${studentInfo.studentName}</p>
                <p class="text-gray-600">Program: ${studentInfo.programName}</p>
                <p class="text-gray-600">Semester: ${studentInfo.semesterName}</p>
            </div>
...
function displayContactSection(container) {
  container.innerHTML += `
